...
/*   Updated: 2022/09/25 14:18:47 by pguranda         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	flood_fill(char **tab, t_vector size, t_vector cur)
{
	if (cur.y < 0 || cur.y >= size.y || cur.x < 0 || cur.x >= size.x \
		...
		return ;
	tab[cur.y][cur.x] = 'X';
	flood_fill(tab, size, (t_vector){cur.x - 1, cur.y, .is_found = FALSE});
	flood_fill(tab, size, (t_vector){cur.x + 1, cur.y, .is_found = FALSE});
	flood_fill(tab, size, (t_vector){cur.x, cur.y - 1, .is_found = FALSE});
	flood_fill(tab, size, (t_vector){cur.x, cur.y + 1, .is_found = FALSE});
}
